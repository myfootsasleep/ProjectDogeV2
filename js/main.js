...
(function($) {
  ...
  $(function() {
    ...
    $(document).ready(function() {
      ...
      // Find the filter controls within the portfolio section
      var filters = $('.filter-control li');
    
      // Initialize Isotope for the portfolio grid
      grid.imagesLoaded(function() {
        grid.isotope({
          itemSelector: '.single-item', // Adjust this selector to match your portfolio item class
          layoutMode: 'fitRows', // Adjust layout mode based on your preference
          masonry: {
            horizontalOrder: true // Adjust masonry options if necessary
          }
        });
      });
    
      // Set up click event listener for filter controls
      filters.on('click', function() {
        // Remove 'tab-active' class from all filters
        filters.removeClass('tab-active');
        // Add 'tab-active' class to the clicked filter
        $(this).addClass('tab-active');
        ...
        grid.isotope({
          filter: selector,
          transitionDuration: '.25s' // Adjust transition duration if necessary
        });
      });
    });
